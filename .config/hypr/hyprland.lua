...
local hostname = io.popen("/bin/hostname"):read("*l")

if hostname == "harry-arch-laptop" then
	hl.monitor({
		output = "eDP-1",
		mode = "preferred",
		position = "auto",
...
local browser = "app.zen_browser.zen || zen-browser"
...
hl.on("hyprland.start", function()
	hl.exec_cmd("systemctl --user start hyprpolkitagent & swaync & waybar & hyprpaper & gammastep-indicator")
	hl.exec_cmd('hostnamectl chassis | grep -qE "laptop|notebook|portable" && hypridle')
...
hl.config({
	general = {
		...
		border_size = 2,
		col = {
			active_border = { colors = { "rgb(5994a6)", "rgb(fbcf92)" }, angle = 45 },
